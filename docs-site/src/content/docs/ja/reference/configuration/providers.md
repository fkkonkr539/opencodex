...
| `requestPacing?` | `{ enabled, requestsPerMinute?, minIntervalMs?, maxConcurrentRequests?, models? }` | 上流の使用量、請求、レート制限表示とは別の、クライアント側の送信リクエスト調整です。`maxConcurrentRequests` はこのプロバイダーへのリクエストが同時に実行できる数の上限で、上流のレスポンス本文が閉じるまでカウントされます。読み取りもキャンセルもされない本文は 30 秒後にリースを返します。プロバイダー制限は全モデルに適用され、`models` は上流の正確なモデル ID に一致し、遅延を増やすか同時上限を厳しくする場合のみ有効です。キュー待機は応答ヘッダーのタイムアウトを消費しません。HTTP、明示的なアダプターの `fetchResponse` 送信と `runTurn` 試行を対象にします。同時上限を設定すると、リースを返せるよう Responses WebSocket は HTTP/SSE に切り替わります。 |
