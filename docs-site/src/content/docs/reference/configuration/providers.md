...
| `requestPacing?` | `{ enabled, requestsPerMinute?, minIntervalMs?, maxConcurrentRequests?, models? }` | Optional client-side outbound request pacing, separate from upstream usage, billing, and rate-limit indicators. RPM is converted to an even interval; `minIntervalMs` may impose a longer interval. `maxConcurrentRequests` caps how many requests to this provider may be in flight at once, counted until the upstream response body closes (a body that is never read or cancelled returns its lease after 30 seconds); a `models` entry may set a tighter per-model cap. Provider limits apply across all models, while `models` entries use exact upstream model IDs (for example `nvidia/llama-3.1-nemotron-ultra-253b-v1`) and can only add delay or tighten the concurrency cap. Queue waits do not consume the upstream response-header timeout. HTTP, explicit adapter `fetchResponse` dispatches, and `runTurn` attempts are covered; a configured concurrency cap moves Responses WebSocket turns to HTTP/SSE so the lease can be returned. |
