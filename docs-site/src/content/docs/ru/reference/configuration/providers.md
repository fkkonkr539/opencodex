...
| `requestPacing?` | `{ enabled, requestsPerMinute?, minIntervalMs?, maxConcurrentRequests?, models? }` | Опциональная клиентская регулировка исходящих запросов, отдельная от учёта использования, биллинга и индикаторов rate limit апстрима. `maxConcurrentRequests` ограничивает число одновременно выполняемых запросов к провайдеру и считается до закрытия тела ответа апстрима; тело, которое так и не прочитали и не отменили, возвращает лимит через 30 секунд. Лимит провайдера действует на все модели, а `models` сопоставляется с точными ID моделей апстрима и может только увеличить задержку или сузить лимит одновременности. Ожидание очереди не расходует таймаут заголовков ответа. Поддерживаются HTTP, явные вызовы адаптеров `fetchResponse` и попытки `runTurn`. При настроенном лимите одновременности Responses WebSocket переключается на HTTP/SSE, чтобы лимит можно было вернуть. |
